...
<!-- FIX: I seem to have done something odd with the numbering, but am not sure what. It's not showing as 0.1, 0.2., 0.3 for the section numbers. --LDC -->
...
<!-- NOTE: i removed the bit about REST above because I thought ONC's explanation was weird, poorly phrased, and confusing. It also didn't seem very relevant to this section. --LDC -->

<!-- DISCUSS:
this is very confusingly phrased:

> Once they are modified for specific requirements using FHIR's built-in capabilities, combinations of Resources are brought together in an Implementation Guide to address a specific use case, such as a provider directory or patient-reported outcomes. This structure lends itself well to expansion beyond FHIR's core capabilities.

It's not clear if ONC is talking about apps using FHIR resources to get data (it's not, but the context sounds like it is) or FHIR developers creating implementation guides for their use cases (what it *is* talking about, but you'd only know that if you knew FHIR).

I suggest removing or rephrasing. A possibel rephrase might be something like:
...
<!-- QUESTION: do we want to list turtle here? --LDC -->

<!-- FIX: use a different callout type here. Maybe an info one or a lightbulb one? --LDC -->
...
<!-- DISCUSS: will the expected reader understand why customization and interoperability would be "trade offs"? Is more background needed for this sentence? 

Original sentence:
FHIR balances the trade-off between customization and interoperability with [the 80/20 rule](http://www.healthintersections.com.au/?p=1924):

Also changed "20% of data elements" to be just "data elements" because we're not actually talking data elements w these percentages. It's "80% of relevant systems", not "80% of data elements". The percentage doesn't translate directly to data elements.
--LDC -->
FHIR uses [the 80/20 rule](http://www.healthintersections.com.au/?p=1924): the core FHIR specification includes a data element if 80% of relevant systems use it. The remaining data elements are handled through [FHIR extensions](https://www.hl7.org/fhir/extensibility.html) (discussed below). <!-- This paragraph is quoted in fhir-history.qmd -- any changes here should be synchronized there manually. -->
...
1.  **Adds constraints:** For example, [the Patient resource in base FHIR](https://www.hl7.org/fhir/R5/patient.html) does not require that the `name` element is populated. US Core's profile of Patient <mark>adds a constraint to make `name` required</mark>.[^2]<!-- QUESTION: why is this 'mark'ed? should it be? remove? I don't think i've seen this in any other modules --LDC -->
...
[^4]: [PACIO defines functional status](http://hl7.org/fhir/us/pacio-fs/STU1/index.html#functional-status) as:
    ...
    (Highlights added.)
<!-- 'mark' used again here. should it be? If not, delete 'Highlights added' as well --LDC -->
...
<!-- I finish this section wanting more. Conclusion sentence? --LDC -->
...
+----------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------+
| Loading data into analysis software          | Convert from JSON with known schema and element types; Possibly test analysis code with automatically generated synthetic data in FHIR format.           | Data format at least partially unknown until final extract is delivered. |
+----------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------+
