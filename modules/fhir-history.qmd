...
learning_objectives:
  - question: What are HL7 V2's key limitations?
    answer: HL7 V2's data format was antiquated compared to modern formats like JSON and XML. It also did not support all types of health-related data, and implementations differed enough to inhibit interoperability.
  - question: What are HL7 V3's key limitations?
    answer: V3 addressed the limitations of V2 but was not widely implemented due to its complexity.
  - question: What characteristics of FHIR have led to its success compared to V2 and V3?
    answer: FHIR has succeeded in wide adoption like V2 while achieving more standardized implementations (as was the goal of V3). FHIR also uses non-health-specific standards like JSON, XML, and REST, which makes it substantially easier to use and integrate with outside health-specific software compared to V2. 
---

In addition to FHIR, the [HL7 standards developing organization](https://www.hl7.org/about/index.cfm?ref=nav) also published two standards that predate FHIR: HL7 V2 and V3. The history of these standards and how they relate to FHIR is described below.

## HL7 V2

[HL7 Version 2](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=185) (commonly called "V2" or "HL7 V2") was first released in 1987. It is a messaging standard for exchanging clinical data between systems. Though it is [widely implemented](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=185), V2 does not meet all modern health interoperability needs. For example:

-   It was created before contemporary web-based interoperability technology was developed, making integration into modern computer systems difficult. For example, V2 messages use their own custom data format, requiring purpose-built software to read and write them.
...
-   The V2 specification does not provide enough structure to ensure interoperability across implementations.

-   V2 does not have a standard approach to sending some common types of health data, like [PDFs](http://www.healthintersections.com.au/?p=2207).
...
HL7 Version 3 ("V3", released in 2005) was created to address limitations of V2. It adopted a more standardized and rigorous approach to specifications and data modeling (the [Reference Information Model](https://www.hl7.org/implement/standards/rim.cfm)), and used a general purpose data format ([XML](https://en.wikipedia.org/wiki/XML)).

V3 did not see as widespread adoption as V2, in part due to the complexity V3 introduced compared with V2. However, you may encounter it in the [C-CDA standard](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=492) for clinical documents, which is widely implemented as part of [Meaningful Use criteria](https://www.healthit.gov/topic/standards-technology/consolidated-cda-overview).

## FHIR

FHIR was developed in the early 2010s to provide a modern, more interoperable alternative to V2 that would be more widely adopted than V3.[^1] Like V3, FHIR uses common, general-purpose data format standards (like [JSON](https://json.org) and XML). It also uses a common, general-purpose architecture ([REST](https://en.wikipedia.org/wiki/Representational_state_transfer)) for communication between systems. These design decisions make it much easier for FHIR-enabled systems to integrate with mobile and cloud-based health applications compared to V2.
...
FHIR tries to balance the trade-off between customization and interoperability with [the 80/20 rule](http://www.healthintersections.com.au/?p=1924): the core FHIR specification typically includes data elements only if 80% of relevant systems will implement it. The remaining 20% of data elements are handled through [FHIR extensions](https://www.hl7.org/fhir/extensibility.html). In practice, this means that many common data elements will be implemented consistently across FHIR-enabled systems.
...
The late 2010s saw adoption outside of EHRs. For example, Apple uses FHIR in [HealthKit](https://developer.apple.com/documentation/healthkit/samples/accessing_health_records) for accessing patient health data directly from their providers' EHRs. The three major public cloud vendors ([Amazon](https://aws.amazon.com/blogs/architecture/building-a-serverless-fhir-interface-on-aws/), [Google](https://cloud.google.com/healthcare-api/docs/concepts/fhir), [Microsoft](https://azure.microsoft.com/en-us/products/health-data-services/)) provide FHIR-enabled services as well.

### FHIR releases

FHIR is currently on [R5](http://hl7.org/fhir/directory.html) (as of April 2023), which is the 5th major release of the FHIR specification. ONC provides ([PDF](https://www.healthit.gov/sites/default/files/page/2021-04/FHIR%20Version%20History%20Fact%20Sheet.pdf)) a summary of FHIR releases since the first in 2012:
