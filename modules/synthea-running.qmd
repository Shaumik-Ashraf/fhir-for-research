...
- running synthea
  * include "basic" setup - JAR setup
  * include "developer" setup - getting the code

  - proposed outline:

    * intro
      * 2 ways to run synthea: basic (JAR) and developer (code)
      * Use basic when <bullet point list>
      * Use developer when <bullet point list>
    
    * Basic setup
      * <instructions from the synthea intro>

    * Developer setup
      * <instructions from the customization section 1

    * Configuration
      * command line configs
        * needs more detail than currently exists given your audience's expected experience. 
          * How do they list the command line args so they see the options?
          * You list examples. Also explain a few of them argument by argument.
      * config file
          * where is it?
...
./gradlew build check
```
**Note: if running on Windows, use `.\gradlew.bat` instead of `./gradlew` -- going forward this guide uses only `gradlew` for brevity**. `gradlew` here is a reference to the [Gradle](https://gradle.org/) build tool which Synthea uses.
