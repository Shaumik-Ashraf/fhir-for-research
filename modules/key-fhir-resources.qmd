...
Jump to [the table of key resources](#table-of-key-resources). The rest of this module walks through the `Patient` resource and an [example patient with high blood-glucose levels](#hyperglycemic-patient-example).
:::
<!-- REWRITE section 2 looks at a patient resource. but observation / bundle / capability statement are also looked at briefly. Also, where is the table of key resources? --LDC -->

## Overview of FHIR resources

Resources are the key building blocks of FHIR. A [**FHIR resource**](https://www.healthit.gov/sites/default/files/page/2021-04/Intro%20to%20FHIR%20Resources%20Fact%20Sheet.pdf) is a structure that contains medical information in an unambiguous format. This allows different systems to understand information in the same way,[^1] which is necessary for health systems to accurately share information.
...
The FHIR core v4.3.0 (also known as Revision 4B, or R4B) defines [140 resources](https://hl7.org/fhir/R4B/resourcelist.html) that allow for healthcare interoperability. The [HL7 FHIR website](https://hl7.org/fhir) gives a well-structured summary of the FHIR core:
...
<!--# FIX: link in caption not rendering correctly --LDC -->
...
A FHIR resource may be encoded in different standard formats like [JSON, XML, or RDF](https://hl7.org/fhir/formats.html#wire). JSON, XML, and RDF are used throughout the software industry, not just in healthcare. Using standard formats allows common programming languages and software tools to process FHIR resources.[^2] 
...
<!-- NOTE: changed "library" to "software" because "library" in this context is software jargon --LDC -->
<!-- OPTIONAL: include snippets from JSON / XML / RDF here? --LDC -->
...
-   **Description & Constraints:** A human-targeted explanation of the attribute that may contain constraints that implementers must enforce. Constraints (like "element X must be populated if element Y is missing") may be formally defined through a language called [FHIRPath](https://hl7.org/fhir/fhirpath.html).
...
<!-- FIX "Patient" is sometimes `Patient` and sometimes Patient. The context doesn't seem different enough for these to be inconsistent. Should choose one and go with it. --LDC -->
...
<!-- DISCUSS: 
"Keeping this attribute optional makes the FHIR resource very flexible: a system implementing the Patient resource **must** be able to accommodate this attribute but **cannot** require it."

doesn't this depend on the profile it uses? It could implement a Patient resource profile that requires `active`. --LDC -->
...
As mentioned above, FHIR defines [19 primitive types](https://www.hl7.org/fhir/datatypes.html#primitive) that those with an informatics or data science background should be familiar with.
...
<!-- FIX Whatever you decide "Patient" should be above (with or without ``) should be applied to HumanName --LDC -->
...
<!-- FIX: I find the 1./2. explanations more confusing than just leaving the code unannotated. Consider removing or reworking. --LDC -->

::: {.callout-note collapse="true"}
## The granularity of FHIR

The structure of `HumanName` highlights the granularity of FHIR. `HumanName` allocates seven attributes for a "name" where a traditional database may only allocate one or two fields. Simultaneously, every attribute is optional, leading to flexibility. One may argue that this makes FHIR too ambiguous for practical use, but context-based [implementation guides (IGs)](/modules/30-fhir-data-modeling--real-world-ig/) and [resource profiles](https://hl7.org/fhir/profiling.html) add constraints to resources to make them more applicable.
:::
...
While choice elements provide flexilibity, they add complexity for implementers, including researchers. When working with choice elements, consider the following:
...
-   Unless an element has a `required` binding, do not assume that FHIR data will only include codes from the specified value set. Make sure you perform an exploratory data analysis for any CodeableConcept binding strength other than `required`.
    ...
    For example, the `Patient.maritalStatus` binding is `extensible`: while you *may* see values from [the bound value set](https://www.hl7.org/fhir/valueset-marital-status.html) (like `M` for married), you may also see values from other value sets (like the SNOMED CT concept [`87915002` ("Married (finding)")](https://browser.ihtsdotools.org/?perspective=full&conceptId1=87915002&edition=MAIN/2023-02-28&release=&languages=en)). 
<!-- FIX: is this a good example? If `M` and SNOMED CT `87915002` are equivalent, doesn't `M` have to be used for an extensible codable concept? Maybe change example from "extensible" to "preferred" --LDC -->
:::
...
<!-- FIX: is profile capitalized or not? Seems like it shouldn't be if resource isn't. --LDC -->
...
<!-- FIX: It says "in this case...". is there supposed to be an example capability statement here? --LDC -->
