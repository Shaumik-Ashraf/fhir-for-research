...
The [SMART App Gallery](https://apps.smarthealthit.org/apps) lists several SMART on FHIR applications, which show how SMART on FHIR can be applied to research. Below are some examples.
...
SMART on FHIR can also help researchers integrate data into an EHR that the EHR does not natively support. For example, @watkins2020 discuss how genetic testing could be better-integrated with EHRs using SMART on FHIR and CDS Hooks (see below<!-- FIX: can this be an anchor link? --LDC -->).

### Accessing bulk data

SMART on FHIR facilitated [Bulk Data Access](http://hl7.org/fhir/uv/bulkdata/) (see below<!-- FIX: can this be an anchor link? --LDC -->) may provide a new way for researchers to access population-level data. A researcher may want to gather population data from multiple institutions that do not store their data using a shared common data model<!-- link to section on CDMs? --LDC -->. However, if these EHRs use FHIR, they likely use the widely implemented [US Core FHIR Implementation Guide](https://www.hl7.org/fhir/us/core/). SMART on FHIR would enable researchers to access the data as US Core<!-- FIX phrasing - this doesn't seem the right way to say this --LDC --> across these institutions despite them not storing their data the same way.
<!-- COMMENT re original text: this is tricky. for data model here, i'm assuming you mean the underlying storage model. but the model used in transit (US Core) may also be considered a data model. so then it's like, but they *do* share a data model if they both use US Core... but you meant they already convert their existing *storage* data model into this *transit* data model... Anyways, it's confusing. --LDC -->
...
<!-- FIX: this caption appears above the table, which is confusing. Fix so it appears at the end. --LDC -->
...
SMART on FHIR applications that integrate with an EHR's user interface via SMART App Launch are often *web applications*. SMART on FHIR applications that run without being triggered by user input in an EHR, including applications that use Bulk Data Access, are often *backend applications*<!-- FIX: other title for this than "backend application"? --LDC -->.
...
Also, consider security and privacy early when developing your research approach. Take steps to ensure proper data storage and access as you would with any software that handles sensitive data. In addition to IRB<!--FIX: link or footnote or write out the whole name --LDC --> approval, you may need approval related to privacy and information security. If your institution has not worked with SMART on FHIR before, it may not have a well-defined process for getting the appropriate approvals.<!--FIX: this feels like it needs a follow-up sentence. Like a "In that case, go to <...>.". Would that be the link below? If so, move up to here and rephrase slightly. --LDC -->
