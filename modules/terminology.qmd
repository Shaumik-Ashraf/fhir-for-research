...
LOINC creates three different human-readable descriptions for each of its concepts [@loinc_term_basics]<!--FIX: broken link --LDC -->:
...
Below is an example of how SNOMED might represent a condition (acute post-streptococcal glomerulenephritis) and its associated relationships: <!--# FIX for Blake to do: Add citation for diagram: adapted from "Georgia Tech Online Health Informatics MS course video" —MM 3/23/23 11:50 AM -->
...
<!--FIX I know this comes direclty from the ICD-10 site, but it doesn't follow the rules listed above.

`S62.630B` -> "B" isn't in the list of possible letters for an extension.
`S62.630D` -> "D" is listed as an "initial encounter" rather than a "subsequent encounter"

`S62.630A`, `S62.630B`, `S62.630D` are all *different* injuries: closed fracture / open fracgture / fracture with routine healing. S62.630* should be all the *same* injury with the letter only specifying the encounter type.

The list above should be changed to align with reality.
... or maybe this is why there's an ICD-11 coming out :D
--LDC -->
...
<!--FIX add "structure" section to this. --LDC -->
