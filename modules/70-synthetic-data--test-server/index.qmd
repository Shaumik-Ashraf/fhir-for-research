...
This module describes a the following options for hosting FHIR data:

- Logica Sandbox
- Docker
- Public Test Servers

While the options listed here may not be robust enough to be production capabilities out of the box<!-- "listed here may not be robust enough to be production capabilities out of the box" seems like it's missing a word --LDC -->, they offer the quickest and easiest options for setting up a FHIR server for testing and training. The options on this page often include a set of default patient data which can be used for basic testing, or you can load your own data created by Synthea.
