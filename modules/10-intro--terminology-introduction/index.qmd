...
## 1. What is terminology in health informatics?
...
At its most basic definition, terminology in IT refers to a code mapping to a concept. A **concept** is a unitary mental representation of a real or abstract thing<!--# FIX what does "unitary mental representation of a real or abstract thing" mean? Rephrase and simplify. --LDC -->, e.g., a disease state or medical procedure. A **code** is a series of letters, numbers, or symbols *assigned to one concept in a code system* for the purposes of classification or identification. A concept is universal and may have a different code between different **code systems**. It is common in healthcare to map codes between multiple code systems to help achieve interoperability between disparate systems.
...
You use a lot of block quotes.
My take: only include large quotes where they state something different/unique from what you were saying and they say it in the best way possible. Otherwise, summarize/reframe/simplify for your audience and cite the original.

--LDC
-->

You might be familiar with some terminology systems like ICD-10<!-- #FIX add link --LDC --> for classifying diseases (and related concepts). These systems typically consist of:
...
## 2. Why is terminology important for interoperability?

[HIMMS summarizes the importance of standard terminology in health informatics](https://www.himss.org/terminology-standards) below:
...
The developers of FHIR were aware of the importance of the ability to translate information between these terminologies, developing multiple Resources specifically for that purpose. Before diving into how that works, we will first review some of the most common terminology systems that you as a researcher might encounter.

## 3. What are some common terminology systems you may encounter?
...
LOINC is a common language for identifying health measurements, observations, and documents. It was created and is maintained by the **Regenstrief Institute**, which describes LOINC as representing the "question" that an observation asks, with other standards (such as SNOMED CT, described next) representing the "answer." It [summarizes LOINC](https://loinc.org/get-started/) as "the global lingua franca for identifying tests and observations."
...
A **concept** represents a unique clinical meaning that is referenced with a unique numeric identifier that is computer readable. A concept also has a **description**, which represents the human readable phrases that describe the concept (what a doctor would read). A given concept can have one or more descriptions linked to it.

Concepts can then be interrelated together with **relationships**, which provide computer readable definitions of concepts and allow for data to be searched for in a variety of ways. Since healthcare workers often have different ways of saying the same thing, relationships allow synonyms to be linked together around a single concept, allowing computer systems to understand the common meaning. There are two major types of relationships:
...
Below is an example how SNOMED might represent a condition and its associated relationships. A patient has acute post-streptococcal ![](images/image-588201257.png)glomerulenephritis, which is then associated with a multitude of hierarchical and attribute relationships.

\*NOTE: let's recreate this in HTML down the road. It is the best visual I've been able to find for showing SNOMED relationships, i got it from this snippet of a [Georgia Tech Online Health Informatics MS course video](https://www.youtube.com/watch?v=OqFILtre8Dc).
<!--# FIX
...
![](images/image-1259988036.png)<!--# FIX broken image link --LDC -->

### 3.3 ICD

#### 3.3.1 About

ICD is a medical classification list developed by the **World Health Organization (WHO)** that contains codes for diseases, signs and symptoms, abnormal findings, complaints, social circumstances, and external causes of injury or diseases. [The WHO describes its impact](https://www.who.int/standards/classifications/classification-of-diseases) as the following, which details direct benefits to research:
...
The CDC's National Center of Immunization and Respiratory Diseases (NCIRD) develops and maintains the CVX code set. Further [details are provided by NCIRD](https://www2a.cdc.gov/vaccines/iis/iisstandards/vaccines.asp?rpt=cvx):

> It includes both active and inactive vaccines available in the US. CVX codes for inactive vaccines allow transmission of historical immunization records. When a MVX (manufacturer) code is paired with a CVX (vaccine administered) code, the specific trade named vaccine may be indicated.
...
## 4. How is terminology implemented in FHIR?

FHIR provides a [Terminology Service](https://build.fhir.org/terminology-service.html#:~:text=A%20FHIR%20terminology%20service%20is,inherently%20known%20terminologies%20providing%20support.) to allow the mapping of information between different terminology standards. It does so by defining three major terminology-related Resources: `CodeSystem`, `ValueSet`, and `ConceptMap`. The diagram below shows how these resources can be used together to map terminology information using FHIR. The next sections will provide more information about each resource.
...
The [`CodeSystem` Resource](https://build.fhir.org/codesystem.html) defines an organized, managed collection of *codes* that belong to a particular terminology standard. Examples include LOINC, SNOMED CT, and the other standards discussed in the previous section. This Resource declares not only the existence of a code system, but also its key properties:
...
::: {.callout-caution collapse="true"}
## Expand to view an example of a simple `CodeSystem` resource
...
The [`ValueSet` Resource](https://build.fhir.org/valueset.html) species a set of codes drawn from one or more code systems intended for use in a particular context. It uses the `CodeSystem` resource by directly referring to it based on whatever code system it is specifying codes from.

::: {.callout-caution collapse="true"}
## Expand to view an example of a `ValueSet` Resource that specifies the value set of "All CPT Values" from the code system SNOMED CT
...
The [`ConceptMap` Resource documentation](https://build.fhir.org/conceptmap.html) provides an overview of both its function as well as its relevance to researchers using FHIR for data analysis involving terminology standards:

> A concept map defines a mapping from a set of concepts defined in a code system (commonly referred to as the "system") to one or more concepts defined in other code systems. In the mapping context, a system can be a typical [code system](https://build.fhir.org/terminologies.html) based on a recognized standard or local terminology (in any of its forms), or in some cases it may be an ["implicit" code system](https://build.fhir.org/conceptmap.html#implicit) that is not based on a recognized terminology but still represents a set of "concepts" that can be usefully mapped. Mappings are one way - from the source to the target system. In many cases, the reverse mappings are valid, but this cannot be assumed to be the case.
>
...
::: {.callout-caution collapse="true"}
## Expand to view an example of a `ConceptMap` Resource that maps SNOMED CT to ICD-10
