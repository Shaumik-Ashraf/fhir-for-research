...
## 1. Terminology in health informatics
...
-->
```
Terminology in Health Informatics is a mapping between a **concept** and a **code**. A concept is a mental representation of a real or abstract thing<!--# FIX what does "unitary mental representation of a real or abstract thing" mean? Rephrase and simplify. --LDC -->, e.g., a disease state or medical procedure. A code is a series of letters, numbers, or symbols *assigned to one concept in a code system* for the purposes of classification or identification. A concept is universal and may have a different code between different **code systems**. Mapping codes between code systems helps achieve interoperability between health systems.
...
## 2. Why terminology is important for interoperability
...
## 3. Common terminology systems you may encounter
...
A **concept** represents a unique clinical meaning that is referenced with a unique computer readable numeric identifier. A concept also has a **description**, which represents the human readable phrases that describe the concept (what a doctor would read). A given concept can have one or more descriptions linked to it.
...
## 4. How terminology is implemented in FHIR
...
The [`ValueSet` Resource](https://build.fhir.org/valueset.html) specifies a set of codes drawn from one or more code systems intended for use in a particular context. It uses the `CodeSystem` resource by directly referring to it based on whatever code system it is specifying codes from.
