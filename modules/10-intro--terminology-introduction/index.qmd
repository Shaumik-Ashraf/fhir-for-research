...
At its most basic definition, terminology in IT refers to a code mapping to a concept. A **concept** is a unitary mental representation of a real or abstract thing, e.g., a disease state or medical procedure. A **code** is a series of letters, numbers, or symbols *assigned to one concept in a code system* for the purposes of classification or identification. A concept is universal and may have a different code between different **code systems**. It is common in healthcare to map codes between multiple code systems to help achieve interoperability between disparate systems.
...
-   **SNOMED CT** (Systemetized Nomenclature of Medicine--Clinical Terms)
...
| Method              | (Optional) A high-level classification of how the observation was made. Only needed when teh technique affects the clinical interpretation of the results. | Manual Count                   |
...
-   **Attribute Relationships**: defines non-hierarchical relationships between two concepts; each concept can ahve as many attribute relationships as needed
...
3.  **Assign concept unique identifier (RXCUI)**: The RXCUI is a numeric identifer that is a code assigned to a given concept. For example, the RXCUI for the above concept example is `198013`.
...
Below is an example of the first 5 entries of the CDX codeset, organized in ascending order of CVX Code:
...
The [`ConceptMap` Resource documentation](https://build.fhir.org/conceptmap.html) provides an overview of both its function as well as its relevance to researchers using FHIR for data analysis involving terminolgy standards:
