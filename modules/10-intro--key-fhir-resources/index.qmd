...
A resource may be encoded in [JSON, XML, or RDF](https://hl7.org/fhir/formats.html#wire) with processing done by any programming language or software tool. The critical concepts across the FHIR resource design are:
 ...
 - metadefinitions for autodiscovery and self-documentation
...
[![The structure for FHIR patient resource.](images/fhir-patient-structure.png)](https://hl7.org/fhir/patient.html#resource)
...
![The structure for FHIR observation resource.](images/fhir-observation-structure.png){fig-alt="Observation structure with attributes status:code, code:CodeableConcept, subject:Reference(Patient), and value\[x\]."}
<!--# FIX this seems to be smaller text, etc., than the previous table example. Can this be made the same size/resolution? --LDC -->
<!--# DISCUSS I followed the same screenshot style from the style guide. --SA -->
...
You may discover certain FHIR communities using **NDJson**, or Newline-Delimited JSON in lieu of `Bundle`. In NDJSon each FHIR resource is printed on one line, and a new line will indicate the next FHIR resource. NDJson is more efficient than Bundle when it comes to very large amounts of data transfer, but as of March 1st 2023 [FHIR R5 standard NDJson use is in Draft status](http://hl7.org/fhir/5.0.0-draft-final/nd-json.html). Bundle is used far more frequently, and sometimes with [pagination](https://en.wikipedia.org/wiki/Pagination) for large amounts of data.
:::
